...
export default function AppleSidebar() {
  ...
  return (
    <div className="h-screen w-64 flex flex-col fixed border-r border-gray-200/20 dark:border-white/10 window-container">
      {/* Profile Section */}
      <div className="p-6 flex flex-col items-center">
        <div className="relative w-24 h-24 mb-4 overflow-hidden rounded-full shadow-xl ring-2 ring-white/10">
        ...
        <h2 className="text-xl font-semibold mb-1 tracking-tight">{portfolio.name}</h2>
        <p className="opacity-60 text-sm mb-4">@gilanglahat22</p>
        
        {/* Social Links */}
        <div className="flex space-x-3 mb-4">
          <a href={portfolio.githubUrl} target="_blank" rel="noopener noreferrer" 
             className="bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 p-2 rounded-full transition-colors backdrop-blur-md">
        ...
        <button className="w-full border border-lime-200/40 bg-lime-200/10 text-white font-medium py-2 px-4 rounded-full transition-all transform hover:scale-105">
          Hire me
        </button>
      </div>
                ...
                >
                  ...
                  {isActive && (
                    <span className="ml-auto">
                      <span className="w-2 h-2 bg-lime-200 rounded-full block"></span>
                    </span>
                  )}
                </Link>
              </li>
      ...
      <div className="border-t border-white/10 p-4 text-xs text-gray-500">
        <div className="flex justify-between items-center mb-4">
          ...
          <button 
            onClick={toggleTheme} 
            className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${theme === 'dark' ? 'bg-lime-200/30' : 'bg-white/20'}`}
          >
